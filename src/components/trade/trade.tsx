...
export default function TradeBox({option, blocks, update, setUpdate, blockType, slipp} : {option:any,
 ...
 const [coinInputTwo, setCoinInputTwo] = useState<any>({metadata: {
	  name: "Tezos",
	  symbol: "TEZ",
	  thumbnailUri: XTZ,
          }, contractAddress: "tez"});
 ...
 const switchTokens = () => {
   const coinOne = coinInputOne;
   const coinTwo = coinInputTwo;
   setCoinInputOne(coinTwo);
   setCoinInputTwo(coinOne);
 }
  ...
  <>
   ...
   {popupOne == true && popupTwo == false && (
	...
	<p className="trade-component-converted-price" key={coinInputTwoUsd}>${(coinInputTwoUsd).toFixed(2)}</p>
      ...
      {option === "send" &&
          <div className="trade-component-send-input-container">
	    <input className="trade-component-send-input" placeholder={"tz address or domain"} />
	  </div>
	}
      {option === "liquidity" && (
      <div className="trade-component-input-container">
